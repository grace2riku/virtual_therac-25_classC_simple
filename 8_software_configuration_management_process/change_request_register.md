...
| CR-0013 | 2026-05-20 | 開発者A | MINOR | CLOSED | 利用者からの依頼で、CR-0006〜0012 の実績に基づくリリース後保守ワークフローのオンボーディングガイド `ONBOARDING.md`(文書 ID `ONB-TH25S-001`、新規プレフィックス ONB-)を構成管理下に追加。GitHub Issue 起点ではない内部のプロセス文書整備(CR-0001 と同様 Issue なし起票)。メタデータヘッダ + 改訂履歴を付与し CIL に CI-DOC-ONB として登録。README §略語に ONB を追加、CLAUDE.md の ID プレフィックスに ONB- を登録。製品コード・SOUP・実装・RCM・要求事項本体・既存規格文書への影響なし(CCB §5.2 基準で MINOR、プロセス補助文書の新規追加)。 | — (利用者からの直接依頼。保守プロセスの教材性向上) | CI-DOC-ONB(新規 `ONBOARDING.md` / ONB-TH25S-001 v1.0), CI-DOC-ACL, CI-DOC-CRR, CI-DOC-CIL, CI-DOC-STEPS。平文文書 README.md / CLAUDE.md(バージョン据え置き) | — (プロセス補助文書の追加、RCM 影響なし) | `docs/cr-0013-onboarding-guide` ブランチ | markdownlint 0 エラー / lychee `--offline --include-fragments` 0 エラー | 2026-05-20 |
| CR-0014 | 2026-05-20 | 開発者A | MINOR | CLOSED | 利用者からの質問「CIL §3 でソースコードを 3 つの CI(CI-SRC-001〜003)に分けている分割基準は何か」に回答し、その Q&A を `IEC62304_QA.md` に QA-003 として追記。分割軸が機能ユニットではなく成果物種別・独立した変更スコープであること(コアライブラリ=ARCH-001 項目境界で 1 CI、ビルド構成=構成ドキュメンテーションとして分離、ルート vs ライブラリ局所で 2 分割)を SAD §4.3 / SCMP §3.3 / IEC 62304 8.1.3 を根拠に整理。回答準備中に判明した「CI 粒度決定基準の未明文化」ギャップは利用者判断により文書是正せず QA-003 内に改善候補として記録するに留めた。製品コード・SOUP・実装・RCM・要求事項本体・既存規格文書への影響なし(CCB §5.2 基準で MINOR、Q&A 教材の追記)。 | — (利用者からの直接質問。GitHub Issue 起点ではない。QAR-TH25S-001 QA-003 として記録) | CI-DOC-QA(QA-003 追記で 1.2 → 1.3), CI-DOC-ACL, CI-DOC-CRR, CI-DOC-CIL, CI-DOC-STEPS | — (Q&A 教材の追記、RCM 影響なし) | `docs/cr-0014-qa003-ci-granularity` ブランチ | markdownlint 0 エラー / lychee `--offline --include-fragments` 0 エラー | 2026-05-20 |
| CR-0015 | 2026-05-20 | 開発者A | MINOR | CLOSED | CR-0014(QA-003)のレビュー過程で検出した CIL の記録ギャップを是正。ルートビルド構成 `CMakeLists.txt`(CI-SRC-002)は CR-0001 で試験フレームワーク選択オプション `TH25S_TEST_FRAMEWORK` が追加されている(同ファイル L41-46 のコメントに「CR-0001 で導入」と明記)にもかかわらず、CIL §3 の CI-SRC-002 行に CR-0001 注記が欠落していた(対照的に試験側 CI-TD-004 は当時から注記済み)。これは CR-0001 当時の CIL §3 反映漏れであり、CIL §3 CI-SRC-002 に CR-0001 注記を補完して是正。CIL §10 1.1 の履歴行(CR-0001 反映)は当時の記録として保持し、CIL §10 1.15 行に遡及是正を記録。ソースコードの実体・CI-SRC-002 の現行バージョン `v1.0.0`・製品挙動・SOUP・RCM への影響なし(CCB §5.2 基準で MINOR、CIL の記録精度の是正のみ)。 | — (CR-0014 のレビューで自己検出。GitHub Issue 起点ではない) | CI-DOC-CIL(§3 CI-SRC-002 注記補完), CI-DOC-ACL, CI-DOC-CRR, CI-DOC-STEPS | — (CIL の記録是正、RCM 影響なし) | `docs/cr-0015-cil-srcc002-cr0001-annotation` ブランチ | markdownlint 0 エラー / lychee `--offline --include-fragments` 0 エラー | 2026-05-20 |
| CR-0016 | 2026-05-20 | 開発者A | MINOR | CLOSED | 利用者からの依頼で、オンボーディングガイド `ONBOARDING.md`(ONB-TH25S-001)§9 実績に CR-0013〜0015 を追記。GitHub Issue 起点ではない 3 件(本書追加 CR-0013 / QA-003 追記 CR-0014 / CIL CI-SRC-002 注記補完 CR-0015)を実績表に反映し、「学びの一例 2(後続作業が記録漏れを炙り出す)」を追加、導入文を以降の保守も本手順で実施している旨に更新。手順本体(§0〜§8)は不変。ONB を 1.0 → 1.1 へ昇格。製品コード・SOUP・実装・RCM・要求事項本体・既存規格文書への影響なし(CCB §5.2 基準で MINOR、プロセス補助文書の実績更新)。 | — (利用者からの直接依頼。GitHub Issue 起点ではない) | CI-DOC-ONB(§9 実績追記で 1.0 → 1.1), CI-DOC-ACL, CI-DOC-CRR, CI-DOC-CIL, CI-DOC-STEPS | — (プロセス補助文書の実績更新、RCM 影響なし) | `docs/cr-0016-onboarding-track-record` ブランチ | markdownlint 0 エラー / lychee `--offline --include-fragments` 0 エラー | 2026-05-20 |
| CR-0017 | 2026-05-20 | 開発者A | MINOR | CLOSED | 利用者からの質問「SDD §4 UNIT-001 のデータ構造で `th25s_error_t` の値域が『OK 他 9 種』と要約されているのに、後続のユニットテスト計画で実装・テストができたのはなぜか/9 種を書かない意図は何か」に回答し、その Q&A を `IEC62304_QA.md` に QA-004 として追記。データ構造表は型の一覧(データ辞書)であり、各エラーコードの正本は §4 の関数仕様表・アルゴリズム・例外表に発生源と一対で分散規定されていること、UTPR §7.2 UT-001-20 が全 10 コードを明示列挙して検査していることを整理。調査で `TH25S_ERR_NOT_READY` が SDD 未記載かつ実装でどの関数も返さない予約コードである点も判明し記録。改善余地(データ構造表の全種列挙・NOT_READY の予約注記)は利用者判断により SDD 是正せず QA-004 内に改善候補として記録するに留めた。製品コード・SOUP・実装・RCM・要求事項本体・既存規格文書への影響なし(CCB §5.2 基準で MINOR、Q&A 教材の追記)。 | — (利用者からの直接質問。GitHub Issue 起点ではない。QAR-TH25S-001 QA-004 として記録) | CI-DOC-QA(QA-004 追記で 1.3 → 1.4), CI-DOC-ACL, CI-DOC-CRR, CI-DOC-CIL, CI-DOC-STEPS | — (Q&A 教材の追記、RCM 影響なし) | `docs/cr-0017-qa004-sdd-error-enum` ブランチ | markdownlint 0 エラー / lychee `--offline --include-fragments` 0 エラー | 2026-05-20 |
| CR-0018 | 2026-05-20 | 開発者A | MINOR | CLOSED | 利用者からの依頼で、オンボーディングガイド `ONBOARDING.md`(ONB-TH25S-001)§9 実績に CR-0016(本書 §9 実績更新)・CR-0017(QA-004 追記)を追記。あわせて導入文の対象範囲を「CR-0014 以降」と一般化(以後の追記で陳腐化しないため)。手順本体(§0〜§8)・学びの一例は不変。ONB を 1.1 → 1.2 へ昇格。製品コード・SOUP・実装・RCM・要求事項本体・既存規格文書への影響なし(CCB §5.2 基準で MINOR、プロセス補助文書の実績更新)。 | — (利用者からの直接依頼。GitHub Issue 起点ではない) | CI-DOC-ONB(§9 実績追記で 1.1 → 1.2), CI-DOC-ACL, CI-DOC-CRR, CI-DOC-CIL, CI-DOC-STEPS | — (プロセス補助文書の実績更新、RCM 影響なし) | `docs/cr-0018-onboarding-track-record-2` ブランチ | markdownlint 0 エラー / lychee `--offline --include-fragments` 0 エラー | 2026-05-20 |
| CR-0019 | 2026-05-20 | 開発者A | MINOR | CLOSED | 利用者からの質問「仕様書・設計書の用語・略語の表記ゆれを検出する仕組みの案は何か(IEC 62304 略語・Therac-25 固有用語として承認された辞書にない表記の検出が目的)」に回答し、その Q&A を `IEC62304_QA.md` に QA-005 として追記。(1) 正本辞書 + (2) CI 機械照合の 2 層構成、許可リスト/禁止リスト/文字正規化の 3 方式比較、本プロジェクトへの層状アプローチ(既存 README §略語 を土台に docs-check.yml の Date format policy ジョブと同型の grep ジョブを増設)、辞書の SSoT 化(glossary.yml)、IEC 62304 §5.2.6 / Therac-25 RCM-004 との関係を整理。CI への実装は利用者判断により行わず、grep ジョブ / textlint+prh / glossary.yml を改善候補として QA-005 内に記録するに留めた。製品コード・SOUP・実装・RCM・要求事項本体・既存規格文書・CI 設定への影響なし(CCB §5.2 基準で MINOR、Q&A 教材の追記)。 | — (利用者からの直接質問。GitHub Issue 起点ではない。QAR-TH25S-001 QA-005 として記録) | CI-DOC-QA(QA-005 追記で 1.4 → 1.5), CI-DOC-ACL, CI-DOC-CRR, CI-DOC-CIL, CI-DOC-STEPS | — (Q&A 教材の追記、RCM 影響なし) | `docs/cr-0019-qa005-terminology-consistency` ブランチ | markdownlint 0 エラー / lychee `--offline --include-fragments` 0 エラー | 2026-05-20 |
| CR-0020 | 2026-05-20 | 開発者A | MINOR | CLOSED | 利用者からの依頼で、オンボーディングガイド `ONBOARDING.md`(ONB-TH25S-001)§9 実績に CR-0018(本書 §9 実績更新)・CR-0019(QA-005 追記)を追記。手順本体(§0〜§8)・導入文・学びの一例は不変。ONB を 1.2 → 1.3 へ昇格。製品コード・SOUP・実装・RCM・要求事項本体・既存規格文書への影響なし(CCB §5.2 基準で MINOR、プロセス補助文書の実績更新)。 | — (利用者からの直接依頼。GitHub Issue 起点ではない) | CI-DOC-ONB(§9 実績追記で 1.2 → 1.3), CI-DOC-ACL, CI-DOC-CRR, CI-DOC-CIL, CI-DOC-STEPS | — (プロセス補助文書の実績更新、RCM 影響なし) | `docs/cr-0020-onboarding-track-record-3` ブランチ | markdownlint 0 エラー / lychee `--offline --include-fragments` 0 エラー | 2026-05-20 |
...
| CR-0013 | MINOR | リリース後保守ワークフローのオンボーディングガイド `ONBOARDING.md`(ONB-TH25S-001)を構成管理下に追加。SOUP・実装・RCM 影響なし | 不要(プロセス補助文書の追加、製品挙動不変) | 不要(学習プロジェクト) |
| CR-0014 | MINOR | QA-003(CIL §3 のソースコード CI 3 分割基準)を `IEC62304_QA.md` に追記。分割軸=成果物種別・変更スコープであることを SAD §4.3 / SCMP §3.3 を根拠に整理。SOUP・実装・RCM 影響なし | 不要(Q&A 教材の追記、製品挙動不変) | 不要(学習プロジェクト) |
| CR-0015 | MINOR | CIL §3 CI-SRC-002 に CR-0001 の `TH25S_TEST_FRAMEWORK` 追加注記を補完(CR-0001 当時の反映漏れの遡及是正)。ソースコード実体・現行バージョン・SOUP・実装・RCM 影響なし | 不要(CIL の記録是正、製品挙動不変) | 不要(学習プロジェクト) |
| CR-0016 | MINOR | オンボーディングガイド `ONBOARDING.md`(ONB-TH25S-001)§9 実績に CR-0013〜0015 を追記(ONB 1.0 → 1.1)。SOUP・実装・RCM 影響なし | 不要(プロセス補助文書の実績更新、製品挙動不変) | 不要(学習プロジェクト) |
| CR-0017 | MINOR | QA-004(SDD UNIT-001 の `th25s_error_t`「OK 他 9 種」記載でも実装・テストできた理由)を `IEC62304_QA.md` に追記。データ構造表は型一覧で正本は関数仕様表に分散、UT-001-20 が全 10 コードを検査することを整理。SOUP・実装・RCM 影響なし | 不要(Q&A 教材の追記、製品挙動不変) | 不要(学習プロジェクト) |
| CR-0018 | MINOR | オンボーディングガイド `ONBOARDING.md`(ONB-TH25S-001)§9 実績に CR-0016・CR-0017 を追記(ONB 1.1 → 1.2)。SOUP・実装・RCM 影響なし | 不要(プロセス補助文書の実績更新、製品挙動不変) | 不要(学習プロジェクト) |
| CR-0019 | MINOR | QA-005(用語・略語の表記ゆれを検出する仕組みの案)を `IEC62304_QA.md` に追記。正本辞書 + CI 機械照合の 2 層、3 方式比較、本 PJ への層状アプローチを整理。CI 実装は伴わず改善候補として記録。SOUP・実装・RCM 影響なし | 不要(Q&A 教材の追記、製品挙動不変) | 不要(学習プロジェクト) |
| CR-0020 | MINOR | オンボーディングガイド `ONBOARDING.md`(ONB-TH25S-001)§9 実績に CR-0018・CR-0019 を追記(ONB 1.2 → 1.3)。SOUP・実装・RCM 影響なし | 不要(プロセス補助文書の実績更新、製品挙動不変) | 不要(学習プロジェクト) |
...
| 2026-05(リリース後 v1.0.0 保守期) | 20 | 1 | 0 | 19 | 0 | 0 |
...
| 1.13 | 2026-05-20 | CR-0013(MINOR・CLOSED)を §4 に追加(オンボーディングガイド ONB-TH25S-001 の構成管理下への追加)。§6 リリース後保守に CR-0013 行を追加。§7 集計表をリリース後保守期 13 件・MINOR 12 件に更新。 | 開発者A |
| 1.14 | 2026-05-20 | CR-0014(MINOR・CLOSED)を §4 に追加(QA-003 = CIL §3 ソースコード CI 3 分割基準の Q&A 記録を IEC62304_QA.md に追記)。§6 リリース後保守に CR-0014 行を追加。§7 集計表をリリース後保守期 14 件・MINOR 13 件に更新。 | 開発者A |
| 1.15 | 2026-05-20 | CR-0015(MINOR・CLOSED)を §4 に追加(CIL §3 CI-SRC-002 への CR-0001 注記補完。CR-0014 レビューで検出した記録漏れの遡及是正)。§6 リリース後保守に CR-0015 行を追加。§7 集計表をリリース後保守期 15 件・MINOR 14 件に更新。 | 開発者A |
| 1.16 | 2026-05-20 | CR-0016(MINOR・CLOSED)を §4 に追加(オンボーディングガイド ONB-TH25S-001 §9 実績に CR-0013〜0015 を追記、ONB 1.0 → 1.1)。§6 リリース後保守に CR-0016 行を追加。§7 集計表をリリース後保守期 16 件・MINOR 15 件に更新。 | 開発者A |
| 1.17 | 2026-05-20 | CR-0017(MINOR・CLOSED)を §4 に追加(QA-004 = SDD UNIT-001 の th25s_error_t「OK 他 9 種」記載でも実装・テストできた理由を IEC62304_QA.md に追記)。§6 リリース後保守に CR-0017 行を追加。§7 集計表をリリース後保守期 17 件・MINOR 16 件に更新。 | 開発者A |
| 1.18 | 2026-05-20 | CR-0018(MINOR・CLOSED)を §4 に追加(オンボーディングガイド ONB-TH25S-001 §9 実績に CR-0016・CR-0017 を追記、ONB 1.1 → 1.2)。§6 リリース後保守に CR-0018 行を追加。§7 集計表をリリース後保守期 18 件・MINOR 17 件に更新。 | 開発者A |
| 1.19 | 2026-05-20 | CR-0019(MINOR・CLOSED)を §4 に追加(QA-005 = 用語・略語の表記ゆれを検出する仕組みの案を IEC62304_QA.md に追記)。§6 リリース後保守に CR-0019 行を追加。§7 集計表をリリース後保守期 19 件・MINOR 18 件に更新。 | 開発者A |
| 1.20 | 2026-05-20 | CR-0020(MINOR・CLOSED)を §4 に追加(オンボーディングガイド ONB-TH25S-001 §9 実績に CR-0018・CR-0019 を追記、ONB 1.2 → 1.3)。§6 リリース後保守に CR-0020 行を追加。§7 集計表をリリース後保守期 20 件・MINOR 19 件に更新。 | 開発者A |
