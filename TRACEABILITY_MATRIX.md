...
**対象ベースライン:** BL-20260520-012(本書作成時点のスナップショット)
...
| SRS-004 | ビームオン要求は READY 状態でのみ受付 | ARCH-001.2 | UNIT-002 | UT-002-04, UT-002-06 | — | ST-001 ※1 | RCM-001 |
...
> **※1 検出された正本間不整合(本統一マトリクス作成 = CR-0009 で発見):** SRS-004 のシステム試験 ID は、[SRS §9](./5.2_software_requirements_analysis/software_requirements_specification.md#9-トレーサビリティマトリクス箇条-526) では `ST-004`、[STPR §11](./5.7_software_system_testing/software_system_test_plan_report.md#11-トレーサビリティマトリクスsrs-網羅性) では `ST-001`(ビームオン到達経路)と **記載が食い違っている**。本表は STPR §11(システム試験トレースの正本)に従い暫定的に `ST-001` とした。この不整合は本書(統一ビュー)の作成によって検出されたもので、各文書の是正は [Issue #11](https://github.com/grace2riku/virtual_therac-25_classC_simple/issues/11) で別途対応する(IEC 62304 §8.2.4 の観点での是正対象。**統一ビューが分散方式の隠れた不整合を検出した実例** である)。
...
>
> **検出された正本間不整合:** 本統一ビューの作成過程で **1 件** 検出した — SRS-004 のシステム試験 ID が SRS §9(`ST-004`)と STPR §11(`ST-001`)で食い違う(§3 ※1 参照)。各文書の是正は [Issue #11](https://github.com/grace2riku/virtual_therac-25_classC_simple/issues/11) で別途対応する。これは「分散方式で各文書が個別にトレース列を持つことの欠点(多重管理による不整合)」を統一ビューが検出した好例である。
