...
> **出典:** RMF §4(HZ / EV)+ RMF §6(RCM / コントロール種別)+ RMF §11(実装・検証)+ SRS §5(SRS-RCM)+ ITPR §13 + STPR §11 を集約。横断行(EV-004 / RCM-004)は RMF §6.1 に基づく(RMF §11 は主要 3 ハザードのみ計上)。
...
| バージョン | 日付 | 変更内容 | 変更者 |
|----------|------|---------|--------|
| 1.0 | 2026-05-20 | 初版作成(CR-0009、Issue #4)。QA-002 §5 の統一マトリクス分離方式の参考実装として、要求駆動(§3)・安全駆動(§4)・RCM 対応(§5)の複合トレーサビリティを各文書から集約。本書は派生ビュー(参考資料)であり正本は各文書である旨を §1 で明示。`BL-20260520-012` 時点のスナップショット。 | 開発者A |
