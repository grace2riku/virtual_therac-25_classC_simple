...
| 1.16 | 2026-05-20 | CR-0016(MINOR)反映: オンボーディングガイド `ONBOARDING.md`(ONB-TH25S-001)§9 実績に CR-0013〜0015 を追記(ONB 1.0 → 1.1)。条項 6(保守)・8(構成管理)・9(問題解決)の運用実績を最新化したプロセス補助文書の更新で、条項対応関係・状態(✅)は不変。CRR/CIL/STEPS の 1.16 昇格に同期。指摘事項なしを維持。 | 開発者A |
| 1.17 | 2026-05-20 | CR-0017(MINOR)反映: 利用者質問への回答 QA-004(SDD UNIT-001 の th25s_error_t「OK 他 9 種」記載でも実装・テストできた理由)を `IEC62304_QA.md` に追記。条項 5.4.2(各ソフトウェアユニットの詳細設計 — クラス C)・5.5(ユニットの検証)に関わり、データ構造表は型一覧で各エラーコードの正本は §4 関数仕様表に分散規定、UT-001-20 が全 10 コードを検査することを Q&A 教材として解説。改善余地(データ構造表の列挙化・NOT_READY 注記)は SDD 是正せず改善候補として記録。Q&A 教材の追記のため条項対応関係・状態(✅)は不変。CRR/CIL/STEPS の 1.17 昇格に同期。指摘事項なしを維持。 | 開発者A |
| 1.18 | 2026-05-20 | CR-0018(MINOR)反映: オンボーディングガイド `ONBOARDING.md`(ONB-TH25S-001)§9 実績に CR-0016・CR-0017 を追記(ONB 1.1 → 1.2)。条項 6(保守)・8(構成管理)・9(問題解決)の運用実績を最新化したプロセス補助文書の更新で、条項対応関係・状態(✅)は不変。CRR/CIL/STEPS の 1.18 昇格に同期。指摘事項なしを維持。 | 開発者A |
| 1.19 | 2026-05-20 | CR-0019(MINOR)反映: 利用者質問への回答 QA-005(用語・略語の表記ゆれを検出する仕組みの案)を `IEC62304_QA.md` に追記。条項 5.2.6(要求事項の検証 — 相互矛盾・曖昧さの排除)・5.1.6 / 5.1.8(ドキュメント)に関わる用語統一の検出手段を Q&A 教材として解説(正本辞書 + CI 機械照合、textlint/prh、glossary.yml の SSoT 化)。CI 実装は伴わず改善候補として記録。Q&A 教材の追記のため条項対応関係・状態(✅)は不変。CRR/CIL/STEPS の 1.19 昇格に同期。指摘事項なしを維持。 | 開発者A |
| 1.20 | 2026-05-20 | CR-0020(MINOR)反映: オンボーディングガイド `ONBOARDING.md`(ONB-TH25S-001)§9 実績に CR-0018・CR-0019 を追記(ONB 1.2 → 1.3)。条項 6(保守)・8(構成管理)・9(問題解決)の運用実績を最新化したプロセス補助文書の更新で、条項対応関係・状態(✅)は不変。CRR/CIL/STEPS の 1.20 昇格に同期。指摘事項なしを維持。 | 開発者A |
