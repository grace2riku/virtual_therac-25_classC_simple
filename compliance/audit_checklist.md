...
| 1.8 | 2026-05-20 | CR-0008(MINOR)反映: 利用者からの要望(GitHub Issue #3)に基づき、Therac-25 ハザードの事故発生メカニズムを Mermaid 図解付きで解説する補助教材 `THERAC25_HAZARD_ANALYSIS.md`(HZA-TH25S-001)を新規作成。条項 7.1(寄与ソフトウェア項目・潜在原因の識別)・5.2.3(リスクコントロールの SRS 組込み)に対応する内容を、ハザード → SRS → RCM → 実装 → 検証のトレースとして教材化(対応関係の正本は RMF/SRS のトレーサビリティマトリクスで不変)。CRR/CIL/STEPS の 1.8 昇格に同期。新規 CI-DOC-HZA は IEC 62304 規定外の補助教材のため適合性評価対象には含めない。条項状態(✅)・指摘事項なしを維持。 | 開発者A |
| 1.9 | 2026-05-20 | CR-0009(MINOR)反映: 利用者からの要望(GitHub Issue #4)に基づき、各文書の §トレース列を集約した統一トレーサビリティマトリクス `TRACEABILITY_MATRIX.md`(UTM-TH25S-001)を参考資料として新規作成。条項 5.2.6 / 5.3.6 / 5.4.4 / 5.5.5 / 5.6.x / 5.7.x の各検証で確認される「トレーサビリティ」を 1 ビューに俯瞰できる参考資料を追加したもので、各条項の対応文書・正本(各文書の分散トレース列)は不変。QA-002 §5 に参照リンク追加(QAR 1.2)。CRR/CIL/STEPS の 1.9 昇格に同期。新規 CI-DOC-UTM は IEC 62304 規定外の参考資料(派生ビュー)のため適合性評価対象には含めない。条項状態(✅)・指摘事項なしを維持。 | 開発者A |
| 1.10 | 2026-05-20 | CR-0010(MINOR)反映: 利用者からの要望(GitHub Issue #5)に基づき、条項 5.4.2(各ソフトウェアユニットの詳細設計 — クラス C)の対応文書 SDD §4 UNIT-002 の状態遷移図を ASCII から Mermaid `stateDiagram-v2` に変更(状態とイベントの視覚的区別を改善、SDD 1.0 → 1.1)。詳細設計の内容(状態機械ロジック・遷移・RCM-001)は不変で表現形式のみの改善のため、条項 5.4.2 の対応関係・状態(✅)は不変。CRR/CIL/STEPS の 1.10 昇格に同期。条項状態(✅)・指摘事項なしを維持。 | 開発者A |
| 1.11 | 2026-05-20 | CR-0011(MINOR)反映: 利用者からの要望(GitHub Issue #6)に基づき、条項 5.4.2(各ソフトウェアユニットの詳細設計 — クラス C)の対応文書 SDD §4 の 3 ユニット(UNIT-001/002/003)の公開 API 表に「関数の責務と引数」小節を追加(各関数の役割・引数・戻り値の意味を `.h` コメントと整合させて明示)。これにより条項 5.4.2 が要求する「各ユニットの詳細設計」の記述が**強化**された(従来は事前/事後条件のみで関数の責務記述が薄かった点を補完)。設計内容・実装・要求は不変。CRR/CIL/STEPS の 1.11 昇格に同期。条項状態(✅)・指摘事項なしを維持。 | 開発者A |
| 1.12 | 2026-05-20 | CR-0012(MINOR)反映: CR-0009 の統一マトリクスが検出した正本間不整合(Issue #11)を是正。条項 5.2.6(要求事項の検証 — トレーサビリティ)・5.7.5(システム試験記録)に関わる SRS §9 と STPR §11 の SRS-004 システム試験 ID の食い違いを、ST-001 を正として SRS §9 を是正し、STPR §8 の ST-001 カバー要求にも SRS-004 を補完して 3 者完全整合させた(SRS 1.2 / STPR 1.1 / UTM 1.1)。要求・試験の実体・RCM・実装は不変でトレーサビリティの正確化のみ。条項対応関係・状態(✅)は不変、むしろトレーサビリティの整合性が向上。CRR/CIL/STEPS の 1.12 昇格に同期。指摘事項なしを維持。 | 開発者A |
| 1.13 | 2026-05-20 | CR-0013(MINOR)反映: リリース後保守ワークフローのオンボーディングガイド `ONBOARDING.md`(ONB-TH25S-001 v1.0)を構成管理下に新設。CR-0006〜0012 の実績に基づく保守手順(条項 6 保守・条項 8 構成管理・条項 9 問題解決の運用面)を 1 文書に集約。プロセス補助文書のため条項対応関係・状態(✅)は不変。CRR/CIL/STEPS の 1.13 昇格に同期。指摘事項なしを維持。 | 開発者A |
| 1.14 | 2026-05-20 | CR-0014(MINOR)反映: 利用者質問への回答 QA-003(CIL §3 のソースコード CI 3 分割基準)を `IEC62304_QA.md` に追記。条項 8.1.1(構成アイテムの識別)・8.1.3(システム構成ドキュメンテーションの識別)の運用解説(製品ソースとビルド構成の CI 種別分離、ARCH-001 項目境界での粒度決定)を Q&A 教材として補強。判明した「CI 粒度基準の未明文化」ギャップは文書是正せず改善候補として記録。Q&A 教材の追記のため条項対応関係・状態(✅)は不変。CRR/CIL/STEPS の 1.14 昇格に同期。指摘事項なしを維持。 | 開発者A |
| 1.15 | 2026-05-20 | CR-0015(MINOR)反映: CR-0014 のレビューで検出した CIL §3 の記録ギャップを是正。条項 8.1.1(構成アイテムの識別)に関わり、ルートビルド構成 CI-SRC-002 が CR-0001 で変更された(`TH25S_TEST_FRAMEWORK` 追加)にもかかわらず CIL §3 に注記が欠落していた点を補完し、構成識別記録の正確性を向上。ソースコード実体・製品挙動は不変、条項対応関係・状態(✅)は不変。CRR/CIL/STEPS の 1.15 昇格に同期。指摘事項なしを維持。 | 開発者A |
