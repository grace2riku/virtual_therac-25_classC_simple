...
TEST_GROUP(SafetyInterlock){};

/* UT-003-01 正常系: 電子モード + 電子位置 + 範囲内パラメータ → TH25S_OK。 */
TEST(SafetyInterlock, ElectronConsistentInputIsAccepted)
{
    th25s_interlock_input_t in = electron_ok();
    LONGS_EQUAL(TH25S_OK, th25s_interlock_check_beam_on(&in));
}

/* UT-003-02 正常系: X線モード + X線位置 + 範囲内パラメータ → TH25S_OK。 */
TEST(SafetyInterlock, XrayConsistentInputIsAccepted)
{
...
TEST(SafetyInterlock, XrayModeWithElectronPositionIsRejected)
{
    th25s_interlock_input_t in = xray_ok();
    in.turntable = TH25S_TT_ELECTRON_POS;
...
TEST(SafetyInterlock, ElectronModeWithXrayPositionIsRejected)
{
    th25s_interlock_input_t in = electron_ok();
    in.turntable = TH25S_TT_XRAY_POS;
    LONGS_EQUAL(TH25S_ERR_MODE_TURNTABLE_MISMATCH,
...
TEST(SafetyInterlock, UnknownTurntableIsRejected)
{
    th25s_interlock_input_t in = electron_ok();
 ...
 * ビーム照射不可) → 拒否。 */
TEST(SafetyInterlock, FieldLightPositionIsRejected)
{
    th25s_interlock_input_t in = xray_ok();
...
TEST(SafetyInterlock, ModeNoneIsRejected)
{
    th25s_interlock_input_t in = electron_ok();
    in.mode = TH25S_MODE_NONE;
    LONGS_EQUAL(TH25S_ERR_MODE_NONE, th25s_interlock_check_beam_on(&in));
}

/* UT-003-08 異常系: エネルギ範囲外 (電子モード 30.0 MeV) → 範囲外として拒否。 */
TEST(SafetyInterlock, EnergyOutOfRangeIsRejected)
{
    th25s_interlock_input_t in = electron_ok();
    in.energy = 30.0;
...
TEST(SafetyInterlock, DoseOutOfRangeIsRejected)
{
    th25s_interlock_input_t in = xray_ok();
    in.dose_cgy = 20000.0;
...
TEST(SafetyInterlock, ElectronLowerBoundEnergyIsAccepted)
{
    th25s_interlock_input_t in = electron_ok();
...
TEST(SafetyInterlock, XrayUpperBoundEnergyIsAccepted)
{
    th25s_interlock_input_t in = xray_ok();
...
TEST(SafetyInterlock, DoseBoundsAreAccepted)
{
    th25s_interlock_input_t in = electron_ok();
...
TEST(SafetyInterlock, NullArgIsRejected)
{
    LONGS_EQUAL(TH25S_ERR_NULL_ARG, th25s_interlock_check_beam_on(NULL));
}

/* UT-003-14 純関数性 (SEP-001): 同一入力に対する複数回呼び出しは同一結果を返す。
 * SafetyInterlock が可変状態を持たない独立判定であることを確認する。 */
TEST(SafetyInterlock, IsPureFunctionForSameInput)
{
    th25s_interlock_input_t ok = xray_ok();
    th25s_interlock_input_t ng = xray_ok();
    ng.turntable = TH25S_TT_ELECTRON_POS;
    for (int i = 0; i < 5; ++i) {
        LONGS_EQUAL(TH25S_OK, th25s_interlock_check_beam_on(&ok));
        LONGS_EQUAL(TH25S_ERR_MODE_TURNTABLE_MISMATCH,
